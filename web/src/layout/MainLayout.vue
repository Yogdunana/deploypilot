...
<template>
  <div class="flex h-screen overflow-hidden bg-background">
    ...
    >
      <aside
        v-if="sidebarOpen"
        class="fixed inset-y-0 left-0 z-50 flex flex-col w-60 border-r border-border bg-card lg:hidden"
      >
        ...
        <div class="flex h-12 items-center justify-between px-4 border-b border-border">
          ...
          <button
            class="inline-flex items-center justify-center w-8 h-8 rounded-md text-muted-foreground hover:bg-accent hover:text-foreground transition-colors cursor-pointer"
        ...
        <ScrollArea class="flex-1 py-2">
          <div v-for="group in navGroups" :key="group.label" class="mb-2">
            <div class="px-4 py-1.5 text-[11px] font-medium uppercase tracking-wider text-muted-foreground">
              {{ group.label }}
            </div>
            <nav class="px-2">
              <RouterLink
                ...
                :to="item.path"
                :class="cn(
                  'flex items-center gap-3 rounded-md px-2 py-2 text-sm transition-colors duration-100',
    ...
    <aside
      :class="cn(
        'hidden lg:flex flex-col border-r border-border bg-card transition-all duration-200 ease-in-out shrink-0',
        collapsed ? 'w-16' : 'w-60'
      )"
    >
      <!-- Logo -->
    ...
    <div class="flex flex-1 flex-col overflow-hidden min-w-0">
      ...
      <header class="flex h-12 items-center justify-between border-b border-border bg-card px-3 sm:px-4 shrink-0">
        <div class="flex items-center gap-2 min-w-0">
          ...
          <button
            class="inline-flex items-center justify-center w-8 h-8 rounded-md text-muted-foreground hover:bg-accent hover:text-foreground transition-colors cursor-pointer shrink-0 lg:hidden"
        ...
        <div class="flex items-center gap-1.5 sm:gap-2 shrink-0">
          <!-- Language Switcher -->
          <button
            class="inline-flex items-center justify-center gap-1.5 rounded-md border border-border bg-background px-2 py-1.5 text-sm text-muted-foreground hover:bg-accent hover:text-foreground transition-colors duration-150 cursor-pointer"
            @click="toggleLocale"
          >
          ...
          <button
            class="inline-flex items-center justify-center gap-2 rounded-md border border-border bg-background px-2.5 sm:px-3 py-1.5 text-sm text-muted-foreground hover:bg-accent hover:text-foreground transition-colors duration-150 cursor-pointer"
            @click="commandOpen = true"
          >
            <Search class="w-3.5 h-3.5" />
            <span class="hidden sm:inline">{{ t('layout.search') }}</span>
            <kbd class="hidden md:inline-flex h-5 items-center gap-1 rounded border border-border bg-card px-1.5 text-[10px] font-medium text-muted-foreground">
